...
class STRAIN_WK(object):
	def __init__(self, 
		...
		# log for the worker
		self.f_log = open(os.path.join(self.base_dir + 'wk_%d_r2sp.txt' % (self.No)), 'w')
		self.f_pre = open(os.path.join(self.base_dir + 'wk_%d_r2sp_pred.txt' % (self.No)), 'w')
		...
		# r2sp: used for adjusting the batch size
	...
	def connect2PS(self):
		...
		if('OK' in msg):
			...
			for v in tf.trainable_variables():
				...
				shape = v.get_shape()
				self.log("Shape: %s\tsize of elements: %f" % (str(shape), sys.getsizeof(v[0])))
				self.para_shape.append(shape)
	...
	def run(self, simulate = 0):
		...
		while (ep < self.max_steps):
			...
			# if(simulate):
			time.sleep(simulate)
			train_X, train_Y, eval_X, eval_Y, test_X, test_Y = self.func_fetch_data(Train = True, Eval = True, Test = True, batch_size = self.final_batch_size)
			...
			if(ep % 10 == 0):
				...
				cur_time = time.time() - self.timer_from_PS	
				self.log('R2SP_WK%d - Step:%d\tTime:%f\tbase_time_step:%ds\tTrain loss: %f\tEval loss: %f'\
					% (self.No, ep, cur_time, self.base_time_step, cur_loss, eval_loss))
			else:
				self.log('R2SP_WK%d - Step:%d\tTime:%f\tbase_time_step:%ds\tTrain loss: %f'\
			...
			# end commit
		#end while
		self.log('R2SP_WK%d stops training - Step:%d\tTime:%f' % (self.No, ep, cur_time))
	...
	def wk_process_commit(self, cur_time, cur_loss, hete_time, time_one_step, cur_step, si, time_span):
		...
		self.log("R2SP_WK%d: Commit %d finished" % (self.No, self.commit_cnt))
		self.log('******************************************************************************************************')
		print
	...
	def wk_process_check(self, cur_time, cur_step):
		...
		# receive new batch size
		self.final_batch_size = int(self.skt.recv(self.buffsize))
		# tf.reshape(self.X, [self.final_batch_size, 32, 32, 3])
		# tf.reshape(self.Y, [self.final_batch_size])
		print
		self.log('########################## check #################################')
		self.log("New batch size: %f" % (self.final_batch_size))	
		self.log('#################################################################')
		print
...
class PSThread(threading.Thread):	
	...
	def return_commit(self):
		global worker_step_list, blocked_time_list, parameters, tau
		# if fastest worker is faster that the slowest worker by more than s clock, hang up
		tmp_time = time.time()
		while True:
			if(max(worker_step_list) == min(worker_step_list) or self.isCheckOrStop == -1):
				break
		blocked_time = time.time() - tmp_time
		blocked_time_list[self.index] += blocked_time
		if(self.isCheckOrStop == -1):
			self.log("Worker %d check stop break" % (self.index))
		else:
			self.log("Worker %d has been blocked for %d seconds" % (self.index, blocked_time))
	...
	def ps_process_check(self, isBlock = True):
		...
		# send batch size, keep the total batch size to the same 
		global worker_num, worker_speed_list, batch_size
		global new_batch_size_list
		# new_batch_size = int((batch_size * worker_num) / (worker_speed_list[self.index] * sum([1.0 / x for x in worker_speed_list])))
		self.skt_client.sendall(str(new_batch_size_list[self.index]))
		self.isCheckOrStop = 0 # normal processing
		self.log('Worker%d\'s new batch size: %d' % (self.index, new_batch_size_list[self.index]))
...
class STRAIN_PS(object):
	def __init__(self,
		...
		global class_num, worker_num, batch_size
		...
		# fro r2sp
		global new_batch_size_list
		new_batch_size_list = [batch_size for _ in xrange(worker_num)]
		...
		# record to files
		self.f_log = open(os.path.join(self.base_dir + 'ps_log_r2sp.txt'), 'w')
		self.f_orig_speed = open(os.path.join(self.base_dir + 'ps_hete_r2sp.txt'), 'w')
		self.f_speed = open(os.path.join(self.base_dir + 'ps_speed_r2sp.txt'), 'w')
		self.f_cmp_time = open(os.path.join(self.base_dir + 'ps_cmp_time_r2sp.txt'), 'w')
		self.f_commit_overhead = open(os.path.join(self.base_dir + 'ps_commit_overhead_r2sp.txt'), 'w')
		self.f_blocked_time = open(os.path.join(self.base_dir + 'ps_blocked_time_r2sp.txt'), 'w')
		self.f_num_local_update = open(os.path.join(self.base_dir + 'ps_num_local_update_r2sp.txt'), 'w')
		
		self.f_global_loss = open(os.path.join(self.base_dir + 'ps_global_loss_r2sp.txt'), 'w')
		self.f_global_eval = open(os.path.join(self.base_dir + 'ps_global_eval_r2sp.txt'), 'w')
		...
		while(not self.trainingEndorNot):
			...
			if(not isfirst):
				...
				if(check_cnt % self.check_period == 0):
					...
					self.record_info()
					global new_batch_size_list
					if(0 not in worker_speed_list):
						speed_sum = sum([1.0 / x if x != 0 else 1 for x in worker_speed_list])
						new_batch_size_list = [int((batch_size * worker_num) / (_speed * speed_sum)) for _speed in worker_speed_list]
					self.log("The %dth check_period; new_batch_size_list: %s" % (check_cnt / self.check_period, str(new_batch_size_list)))
