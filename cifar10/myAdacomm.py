...
class STRAIN_WK(object):
	def __init__(self, 
		...
		# log for the worker
		self.f_log = open(os.path.join(self.base_dir + 'wk_%d_ada.txt' % (self.No)), 'w')
		self.f_pre = open(os.path.join(self.base_dir + 'wk_%d_ada_pred.txt' % (self.No)), 'w')
	...
	def run(self, simulate = 0):
		...
		while (ep < self.max_steps):
			...
			if(ep % 10 == 0):
				...
				cur_time = time.time() - self.timer_from_PS	
				self.log('ADA_WK%d - Step:%d\tTime:%f\tbase_time_step:%ds\tTrain loss: %f\tEval loss: %f'\
					% (self.No, ep, cur_time, self.base_time_step, cur_loss, eval_loss))
			else:
				self.log('ADA_WK%d - Step:%d\tTime:%f\tbase_time_step:%ds\tTrain loss: %f'\
			...
			# end commit
		#end while
		self.log('ADA_WK%d stops training - Step:%d\tTime:%f' % (self.No, ep, cur_time))
	...
	def wk_process_commit(self, cur_time, cur_loss, hete_time, time_one_step, cur_step, si, time_span):
		...
		# receive tau
		self.base_time_step = float(self.skt.recv(self.buffsize))
		...
		self.commit_overhead += time.time() - tmp_time
		self.log("ADA_WK%d: Commit %d finished" % (self.No, self.commit_cnt))
		self.log('******************************************************************************************************')
		print
	...
	def wk_process_check(self, cur_time, cur_step):
		...
		print
		self.log('########################## check #################################')
		self.log("New tau: %f" % (self.base_time_step))
		self.log('#################################################################')
		print
...
class STRAIN_PS(object):
	def __init__(self,
		...
		# record to files
		self.f_log = open(os.path.join(self.base_dir + 'ps_log_ada.txt'), 'w')
		self.f_orig_speed = open(os.path.join(self.base_dir + 'ps_hete_ada.txt'), 'w')
		self.f_speed = open(os.path.join(self.base_dir + 'ps_speed_ada.txt'), 'w')
		self.f_cmp_time = open(os.path.join(self.base_dir + 'ps_cmp_time_ada.txt'), 'w')
		self.f_commit_overhead = open(os.path.join(self.base_dir + 'ps_commit_overhead_ada.txt'), 'w')
		self.f_blocked_time = open(os.path.join(self.base_dir + 'ps_blocked_time_ada.txt'), 'w')
		self.f_num_local_update = open(os.path.join(self.base_dir + 'ps_num_local_update_ada.txt'), 'w')
		
		self.f_global_loss = open(os.path.join(self.base_dir + 'ps_global_loss_ada.txt'), 'w')
		self.f_global_eval = open(os.path.join(self.base_dir + 'ps_global_eval_ada.txt'), 'w')
	...
	def offlineSearchOneS(self, opt_tau, max_dec, tmp_tau, init_loss, backup_para, ps_t):
		...
		if(dec > max_dec):
			opt_tau = tmp_tau
			max_dec = dec
		self.log("	- current opt tau: %f" %(opt_tau))
	...
	def offlineSearchS(self, ps_t):
		...
		# offline search, do not need to judge convergence
		self.loadmodel()
		self.evaluation(10)
		...
		while(not self.trainingEndorNot):
			...
			if(not isfirst):
				...
				if(firstIN):
					...
					self.allStop(ps_t)
					if(not self.FLAGS_Fixed):
						self.log('Adacomm: find the optimal tau')
					...
					self.log("Accu Search Time: %f"%(gridSearchTime))
					self.record_info()
					self.log('#################################################################')
					print
				# end if # epoch
				...
				if(check_cnt % self.check_period == 0):
					self.record_info()
					self.log("The %dth check_period" % (check_cnt / self.check_period))
					# evaluation
					self.get_global_loss()	
					# if run adacomm, adaptively tune tau
					if((not self.FLAGS_Fixed) and (not self.trainingEndorNot)):
						# adacomm update rule
						...
						self.log("ADACOMM Updated tau: %f\tUpdate rule: %d" % (tau, rule))
				#end if
			#end if first
		# end while
		self.log("PS ends")
