...
class STRAIN_PS(object):
	...
	def onlineSearch1MinPlusSi(self, max_mu, max_eta, max_reward, _mu, _eta, last_loss, last_loss_time, ps_t):
		global mu, eta, START_TIME
		# load mu and eta first
		mu = _mu
		eta = _eta

		self.log("	- Current mu: %f\tCurrent eta: %f" % (_mu, _eta))
		...
		if(reward > max_reward):
			...
			max_eta = eta
			max_reward = reward
		self.log("	- reward: %f (last_loss list: %s\t time list: %s" % (reward, str(last_loss), str(last_loss_time)))
		last_loss = [last_loss[-1]]
		last_loss_time = [last_loss_time[-1]]
		print("	- current opt (mu eta): (%f %f)\n##########################" % (max_mu, max_eta))
		return max_mu, max_eta, max_reward, last_loss, last_loss_time

	def onlineSearchMuEta(self, ps_t, c_target, last_loss, last_loss_time):
		'''
			search with one configuration for 1 min
		'''
		global expect_commit, eta
		...
		for _mu in Mu:
			for _eta in Eta:
				if (self.trainingEndorNot): 
					break
				max_mu, max_eta, max_reward, last_loss, last_loss_time = self.onlineSearch1MinPlusSi(
					max_mu, max_eta, max_reward, _mu, _eta,
					last_loss, last_loss_time, 
					ps_t)
		if((not self.trainingEndorNot) and max_mu == 0):
			max_mu, max_eta, max_reward, last_loss, last_loss_time = self.onlineSearch1MinPlusSi(
					max_mu, max_eta, max_reward, 0.1, eta,
					last_loss, last_loss_time,
					ps_t)
			max_mu, max_eta, max_reward, last_loss, last_loss_time = self.onlineSearch1MinPlusSi(
					max_mu, max_eta, max_reward, 0.2, eta,
					last_loss, last_loss_time,
					ps_t)
		return max_reward, last_loss, last_loss_time, max_mu, max_eta
		...
		while(not self.trainingEndorNot):
			...
			if(not isfirst):
				...
				if(forceSearch or (check_cnt - lastSearchCnt >= CHECKPOINT_PER_EPOCH)):
					...
					cur_reward, last_loss, last_loss_time, mu1, eta1 = self.onlineSearchMuEta(ps_t, c_target, last_loss, last_loss_time)
					if(not self.trainingEndorNot): 
						next_reward, last_loss, last_loss_time, mu2, eta2 = self.onlineSearchMuEta(ps_t, c_target+1, last_loss, last_loss_time)
					else:
