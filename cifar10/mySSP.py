...
class STRAIN_WK(object):
	def __init__(self, 
		...
		# log for the worker
		self.f_log = open(os.path.join(self.base_dir + 'wk_%d_ssp.txt' % (self.No)), 'w')
	...
	def run(self, simulate = 0):
		...
		while (ep < self.max_steps):
			...
			tmp_time = time.time()
			self.push(cur_time, cur_loss, hete_time, time_one_step, ep, si, time_span)
			self.commit_overhead += time.time() - tmp_time
			self.log("Time per batch: %f" % (hete_time))
			self.log("Time one step: %f" % (time_one_step))
			self.log("Time from last pull: %f" % (time_span))
			self.log("Time of one step (include push): %f" % (time.time() - time_step_start))
			#  commit to the PS 
			if(ep - self.min_c >= self.s):
				tmp_time = time.time()
				self.pull()
				self.commit_overhead += time.time() - tmp_time
	...
	def push(self, cur_time, cur_loss, hete_time, time_one_step, cur_step, si, time_span):
		self.commit_cnt += 1
		...
		## [IMPORTANT]: here we do not use time_span, use time_one_step to calculate the computation time
		l = "%20.15f, %20.15f, %d, %20.15f, %d, %20.15f" % (hete_time, cur_loss, cur_step, time_one_step, si, time_one_step)
		self.sendmsg(l)
	...
	def pull(self):
		...
		self.log("SSP_WK%d: Commit %d finished" % (self.No, self.commit_cnt))
		self.log('******************************************************************************************************')
		print
...
class PSThread(threading.Thread):	
	...
	def listen(self):
		...
		global push_cnt_list, pull_cnt_list
		while self.go_on:
			msg = self.skt_client.recv(self.buffsize)
			if('Push' in msg):
				push_cnt_list[self.index] += 1
				self.skt_client.sendall("start commit")
				self.recv_push()
			elif('Pull' in msg):
				pull_cnt_list[self.index] += 1
				self.return_pull()
				self.check()
			elif('Exit' in msg):
				break
	...
	def return_pull(self):
		...
		if(self.isCheckOrStop == -1):
			...
			self.log("Worker %d has been blocked for %f seconds" % (self.index, blocked_time))
...
class STRAIN_PS(object):
	def __init__(self,
		...
		global push_cnt_list, pull_cnt_list, expect_commit, commit_overhead_list,blocked_time_list
		global worker_loss_list, original_worker_speed_list, worker_step_list, computation_time_list, worker_speed_list, num_of_local_update
		...
		push_cnt_list = [0 for _ in xrange(worker_num)]
		pull_cnt_list = [0 for _ in xrange(worker_num)]
		...
		# record to files
		self.f_log = open(os.path.join(self.base_dir + 'ps_log_ssp.txt'), 'w')
		self.f_orig_speed = open(os.path.join(self.base_dir + 'ps_hete_ssp.txt'), 'w')
		self.f_speed = open(os.path.join(self.base_dir + 'ps_speed_ssp.txt'), 'w')
		self.f_cmp_time = open(os.path.join(self.base_dir + 'ps_cmp_time_ssp.txt'), 'w')
		self.f_commit_overhead = open(os.path.join(self.base_dir + 'ps_commit_overhead_ssp.txt'), 'w')
		self.f_blocked_time = open(os.path.join(self.base_dir + 'ps_blocked_time_ssp.txt'), 'w')
		self.f_num_local_update = open(os.path.join(self.base_dir + 'ps_num_local_update_ssp.txt'), 'w')
	...
	def record_info(self):
		global START_TIME, push_cnt_list, pull_cnt_list
		cur_time = time.time() - START_TIME
		...
		self.f_log.write('%020.10f: step = %s\tloss= %s\tpush_cnt=%s\tpull_cnt=%s\n' % 
			(cur_time, str(worker_step_list), str(worker_loss_list), str(push_cnt_list), str(pull_cnt_list)))
