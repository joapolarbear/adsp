...
class STRAIN_WK(object):
	...
	def wk_process_commit(self, cur_time, cur_loss, hete_time, time_one_step, cur_step, si, time_span):
		...
		self.skt.sendall(l)
		self.log("The size of (send some trival message): %d M" % (float(sys.getsizeof(l))/(1024.0 * 1024.0)))
...
class STRAIN_PS(object):
	...
	def get_global_loss(self, offline=False):
		self.loadmodel()
		self.evaluation(10)
		if(offline): # used to calculate the loss when offline search 
			return # no need to judge convergence
	...
	def offlineSearchOneCombine(self, ps_t, init_loss, _mu, _eta, opt_config, backup_para, backup_v_para):
		global mu, eta
		mu = _mu
		eta = _eta
		...
		self.get_global_loss(offline=True)
		dec = init_loss - self.global_loss
		if(dec > opt_config[0]): # opt_confit: [reward, mu, eta]
			opt_config = [dec, _mu, _eta]
		global parameters, v_para
		parameters = copy.deepcopy(backup_para)
		v_para = copy.deepcopy(backup_v_para)
		self.record_info()
		self.log('	-- current search: dec=%f, mu=%f, eta=%f' % (dec, _mu, _eta))
		self.log('	-- current opt config: max_dec=%f, mu=%f, eta=%f' % (opt_config[0], opt_config[1], opt_config[2]))
		return opt_config

	def offlineSearchForCtarget(self, ps_t, c_target, init_loss, space_mu, space_eta, backup_para, backup_v_para):
		# config the c_target
		global expect_commit
		expect_commit = [c_target for _ in expect_commit]
		self.log('- Current search: C_target=%f' % c_target)

		# initialize the optimal configure [reward, mu, eta]
		opt_config = [-1000000.0, space_mu[0], space_eta[0]]
		for _mu in space_mu:
			for _eta in space_eta:
				opt_config = self.offlineSearchOneCombine(ps_t, init_loss, _mu, _eta, opt_config, backup_para, backup_v_para)
		if(opt_config[1] == 0):
			opt_config = self.offlineSearchOneCombine(ps_t, init_loss, 0.1, opt_config[2], opt_config, backup_para, backup_v_para)
			opt_config = self.offlineSearchOneCombine(ps_t, init_loss, 0.2, opt_config[2], opt_config, backup_para, backup_v_para)

		self.log('- Optimal config when C_target=%f: max_dec=%f, mu=%f, eta=%f' % (c_target, opt_config[0], opt_config[1], opt_config[2]))
		print
		opt_config.append(c_target)
		return opt_config
		...
		while(not self.trainingEndorNot):
			...
			if(not isfirst):
				...
				if(forceSearch or (check_cnt - lastSearchCnt >= CHECKPOINT_PER_EPOCH)):
					...
					# backup the parameters, and get the initial loss
					global parameters, v_para
					...
					# construct the search space for each c_target
					global mu, eta
					space_mu = [0, 0.3, 0.6, 0.9]
					space_eta = [eta, eta / 10]

					# return [reward, opt_mu, opt_eta, c_target]
					opt = self.offlineSearchForCtarget(ps_t, c_target, init_loss, space_mu, space_eta, backup_para, backup_v_para)
					next_rst = self.offlineSearchForCtarget(ps_t, c_target+1, init_loss, space_mu, space_eta, backup_para, backup_v_para)

					# if next_reward c_target+1 is better, continue to search c_target + 2
					while(next_rst[0] > opt[0]): # next_reward > cur_reward
						opt = next_rst
						c_target += 1 
						next_rst = self.offlineSearchForCtarget(ps_t, c_target+1, init_loss, space_mu, space_eta, backup_para, backup_v_para)
					...
					mu = opt[1]
					eta = opt[2]
					expect_commit = [opt[3] for _ in expect_commit]
					...
					self.log("USP:expect_commit %s\tmu: %f\teta: %f\n" % (str(expect_commit), mu, eta))
					self.log('#################################################################')
					print
				# end if # epoch
