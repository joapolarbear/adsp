...
class STRAIN_WK(object):
	...
	def run(self, simulate = 0):
		...
		while (ep < self.max_steps):
			...
			if((self.notWait and (time_span > self.base_time_step)) or ((not self.notWait) and si > self.base_time_step)):
				...
				msg = self.skt.recv(self.buffsize)
				if(not self.notWait):
					print("Allow waiting, meet the s requirement, stop to commit, check?: %s" %msg)
				else:
					print("Not allow waiting, meet the time requirement, stop to commit, check?: %s"%msg)
	...
	def wk_process_check(self, cur_time, cur_step, test_X, test_Y):
		...
		if(expect_cNo < 0):
			# based on the time
			self.base_time_step = self.check_period / float(- expect_cNo)
			self.notWait = True
		else:
...
class STRAIN_PS(object):
	def __init__(self,
		...
		returnS = [0 for _ in xrange(worker_num)]
	...
	def offlineSearch1MinPlusSi(self, 
					c_target, opt_si, max_mu, max_eta, max_dec, _mu, _eta,
					init_loss, backup_para, backup_v_para,
					ps_t):
		global parameters, v_para
		global mu, eta
		# load mu and eta first
		mu = _mu
		eta = _eta

		print("# Current mu: %f\tCurrent eta: %f" % (_mu, _eta))
		global expect_commit, worker_num
		###################
		## run without waiting, initialize with c_target(commit frequency)
		expect_commit = worker_num * [- c_target]
		cnt = 0
		self.allStart(ps_t)
		...
		if(dec > max_dec):
			...
			max_mu = _mu
			max_eta = _eta
			opt_si = copy.deepcopy(expect_commit)
		...
		global returnS
		print("# Upper bound: %s" % str(returnS))
		#construct search space
		minS = min(returnS)
		searchSpace = [xrange(minS, si+1) for si in returnS]

		searchSize = 1
		sizeL = []
		for i in xrange(worker_num):
			searchSize = searchSize * (returnS[i] - minS + 1)
			sizeL.append(returnS[i] - minS + 1)

		print("# searchSize: %d\t sizeL:%s" %( searchSize, str(sizeL)))

		sizeLevelL = worker_num * [1]
		for i in xrange(worker_num-2, -1, -1):
			sizeLevelL[i] = sizeLevelL[i+1] * sizeL[i+1]
		print("# sizeLevelL:%s" % str(sizeLevelL))
		###################

		granularity = float(max(1, (searchSize / 10)))
		def frange(start, stop, step):
			x = start
			while x < stop:
				yield x
				x += step
		for searchIndex in frange(0.0, float(searchSize), granularity):
			tmp = int(searchIndex)
			expect_commit = []
			for i in xrange(worker_num-1):
				expect_commit.append(tmp / sizeLevelL[i] + minS)
				tmp = tmp % sizeLevelL[i]
				# print("i:%d, tmp:%d" % (i, tmp))
			expect_commit.append(tmp + minS)
			...
			print("## mu: %f\teta: %f => index:%d current search si: %s" % (mu, eta, searchIndex, str(expect_commit)))
			self.loadmodel(parameters)
			self.evaluation(10)
			if(self.global_loss > 10 * init_loss):
				print("Early abort !!!!!")
				parameters = copy.deepcopy(backup_para)
				v_para = copy.deepcopy(backup_v_para)
				break
			...
			parameters = copy.deepcopy(backup_para)
			v_para = copy.deepcopy(backup_v_para)
			print("## current opt (mu eta si): (%f %f %s)" % (max_mu, max_eta, str(opt_si)))

		# expect_commit = copy.deepcopy(opt_si)
		print("# current opt (mu eta si): (%f %f %s)\n##########################" % (max_mu, max_eta, str(opt_si)))
		return max_mu, max_eta, max_dec, opt_si

	# gridSearch for optimal mu (and eta) for current c_target
	def offlineGridSearchPlusSi(self, c_target, Mu, Eta, ps_t, last_mu, last_eta):		
		global expect_commit, parameters, v_para, checkInCnt, worker_num
		# until all workers have reached, go on
		backup_para = copy.deepcopy(parameters)
		backup_v_para = copy.deepcopy(v_para)

		print("\nStart Search => Current Freqency: %d per %fs" % (c_target, self.check_period))
		self.loadmodel(parameters)
		self.evaluation(10)
		...
		for _mu in Mu:
			for _eta in Eta:
				...
				max_mu, max_eta, max_dec, opt_si = self.offlineSearch1MinPlusSi(
					c_target, opt_si, max_mu, max_eta, max_dec, _mu, _eta,
					init_loss, backup_para, backup_v_para,
					ps_t)
		...
		if(max_mu == 0):
			max_mu, max_eta, max_dec, opt_si = self.offlineSearch1MinPlusSi(
					c_target, opt_si, max_mu, max_eta, max_dec, 0.1, eta,
					init_loss, backup_para, backup_v_para,
					ps_t)
			max_mu, max_eta, max_dec, opt_si = self.offlineSearch1MinPlusSi(
					c_target, opt_si, max_mu, max_eta, max_dec, 0.2, eta,
					init_loss, backup_para, backup_v_para,
					ps_t)

		# load the opt configuration
		global mu, eta
		mu = max_mu
		eta = mu / float(worker_num) 
		expect_commit = copy.deepcopy(opt_si)

		self.loadmodel(parameters)
		self.evaluation(10)
		print("Initial Loss: %f\t Final Loss: %f" % (init_loss, self.global_loss))
	...
	def run(self):
		...
		epoch = 1000
		global MOMENTUN
		forceSearch = False # used to avoid divergence
		gridSearchTime = 0.0
		# test
		self.check_period = 60
		c_target = 1
		...
		while True:
			...
			if(not isfirst):
				...
				if(MOMENTUN and (forceSearch or (check_cnt - lastSearch >= epoch))):
					...
					self.offlineGridSearchPlusSi(c_target, Mu, Eta, ps_t, mu, eta)
				...
				if(check_cnt % self.check_period == 0):
					...
					self.loadmodel(parameters)
					self.evaluation(10)
					if((check_cnt / self.check_period >= 6) and (self.global_loss - last_global_loss) / last_global_loss >= 0.3):
						forceSearch = True
