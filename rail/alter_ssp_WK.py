...
flags.DEFINE_integer('port_base', 14450, 'Start port for listening to workers')
...
class Model(object):
	def __init__(self):		
		...
		self.f_log = open(self.base_dir + 'wk_%d_alter_ssp.txt' % (self.No), 'w')
		self.f_pre = open(self.base_dir + 'wk_%d_alter_ssp_pred.txt' % (self.No), 'w')
		# self.mse = 10
		# self.cnt = 0
		# self.ep_period = 100
	...
	def random_sleep(self):
		# sleep_time = 1.0 * (self.No % 3) + 1.0
		# min_sleep_time = 0.5
		# time.sleep(float(sleep_time) * min_sleep_time)
	...
	def run(self):	
		...
		while self.local_c < self.max_steps:
			...
			self.update_class(train_Y.tolist())	# update class_cnt
			...
			if(self.local_c - self.min_c >= self.s):
				self.process_commit(cur_time, cur_loss, hete_time, time_one_step, cur_step)
				# check
			...
			# end commit
		# end for
		print('ALTER_SSP_WK{} stops training - Step:{}\tTime:{}'.format(self.No, self.local_c, cur_time))
	...
	def process_commit(self, cur_time, cur_loss, hete_time, time_one_step, cur_step):
		self.commit_cnt += 1
		...
		# l = "%20.15f, %20.15f, %d" % (cur_time, cur_loss, self.local_c)
		self.skt.sendall(l)
		...
		for v in tf.trainable_variables():
			...
			index += 1
		# print("SSP_WK{}:Commit {} finished".format(self.No, self.commit_cnt))
		print("***************\nALTER_SSP_WK{}:Commit {} finished\n***********".format(self.No, self.commit_cnt))
	...
	# def send2PS(self, s):		
	# 	self.skt.sendall(s)
	# 	self.skt.sendall('\0' * 1024)

	# def sendlist2PS(self, l):
	# 	s = str(l)		
	# 	self.skt.sendall(s)
	# 	self.skt.sendall('\0' * 1024)
